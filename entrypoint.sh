...
su steam -c "cd ${SERVERDIR}/ark/ &&
  ./ShooterGame/Binaries/Linux/ShooterGameServer ${MAP}?listen?Multihome=0.0.0.0?SessionName=${SERVERNAME}?MaxPlayers=${MAXPLAYERS}?ServerPassword=${PASSWD}?ServerAdminPassword=${ADMINPASSWD} -server -log"
